package org.metadatacenter.cedar.template.resources.rest.template;
...
public class UpdateTemplateTest extends AbstractRestTest {
  ...
  @Parameters(method = "getParamsUpdateTemplatePut")
  public void updateTemplateTest(TestParameterArrayGeneratorGenerator generator,
    ...
    ((ObjectNode) template).put(CedarModelVocabulary.SCHEMA_DESCRIPTION, TEST_DESCRIPTION_VALUE);
    String modifiedContent = JsonMapper.MAPPER.writeValueAsString(template);
    ...
    if (expectedResponseStatus == Response.Status.OK.getStatusCode()) {
      ...
      try {
        getTemplate = JsonMapper.MAPPER.readTree(getBody);
      } catch (JsonParseException e) {
        // do nothing, the json can be invalid intentionally
      }
      JsonNode descriptionNode = getTemplate.get(CedarModelVocabulary.SCHEMA_DESCRIPTION);
  ...
  private Object getParamsUpdateTemplatePut() {
    ...
    generatorForTemplate.addParameterValue(1, TEMPLATE, "nodeType");
    generatorForTemplate.registerParameter(2, authHeader, "authHeader");
    generatorForTemplate.registerParameter(3, ids(idInUrl, "nodeType"), "idInUrl");
